...
def get_review_rating_from_file(json_file_path, limit=None):
    """Read in the json dataset file and return the reviews and ratings."""
    reviews = []
    num_processed = 0
    with open(json_file_path) as fin:
        for line in fin:
            ...
            if limit is not None and limit <= num_processed:
                break
    return reviews

def write_file(reviews, reviews_csv_file_path, ratings_csv_file_path):
    """Read in the json dataset file and write it out to a csv file, given the column names."""
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'limit',
        type=int,
        help='Limit of reviews to process',
        default=None,
        nargs='?',
    )
